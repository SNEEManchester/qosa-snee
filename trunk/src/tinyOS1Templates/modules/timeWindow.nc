...
__HEADER__
	...
	void task signalDoneTask()
	{
			...
			dbg(DBG_USR1,"TIME_WINDOW: output cardinality %d tuple(s) for evalTime %d\n",outputTupleCount,currentEvalTime);
		#endif
	...
	void task refreshWindowTask() {
		...
		{
			dbg(DBG_USR1,"TIME_WINDOW: refreshWindowTask currentEvalTime %d < slideAdjust %d + WINDOW_TO %d\n",currentEvalTime,slideAdjust,WINDOW_TO);
		...
		else
		{
			...
			{
				dbg(DBG_USR1,"__MODULE_NAME__  indexPos %d with EvalTime %d ignored\n",indexPos,originalEvalTimes[indexPos]);
				if (windowTail == 0) {
