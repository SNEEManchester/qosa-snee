...
import os, random, RandomSeeder, math, networkLib100, SneeqlLib, sys, getopt, UtilLib
...
def getCandidateNode(nodes, rValue, id):
        ...
        n = networkLib100.Node(id, int(randomNode.xPos + dx), int(randomNode.yPos + dy))
        return n

def generateSpiderNetwork(nSize, nDensity, networkId, outputDir):

    allConnected = False
    retries = 0
    while (not allConnected) and (retries < 100):
		field = networkLib100.generateConnectedRandomTopology(numNodes = nSize, rValue = nDensity, radioRange = 15, sinkAtCenter = False, sinkAtOrigin = True)
    ...
    if not allConnected:
	    ...
	    field.generateSneeqlNetFile(outputDir+os.sep+networkId+".xml")
	    field.generateTopFile(outputDir+os.sep+networkId+".top")
