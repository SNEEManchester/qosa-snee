...
__HEADER__
	...
	{
		__MAX_DEBUG1__ dbg("DBG_USR3","trayGetTuplesDoneTask entered\n");
		__MAX_DEBUG1__ dbg("DBG_USR3","SubTrayNum: %d, trayHead: %d, trayTail: %d\n",subTrayNum,trayHead,trayTail);
		temp = subTrayNum;
		subTrayNum = -1;
		__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__ subtray set to %d\n",subTrayNum);

		signal GetTuples.requestDataDone(tray[temp], trayHead, trayTail, SUBTRAY_SIZE);
	}


	command error_t GetTuples.requestData(int16_t evalEpoch)
	{
		__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__  requestData entered\n");
		...
		__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__  evalEpoch=%d, subTrayNum=%d, pos=%d\n",evalEpoch, subTrayNum,trayTail);
		__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__  evalEpoch at this tray position: %d\n",tray[subTrayNum][trayTail].evalEpoch);
		...
		while ((trayTail<SUBTRAY_SIZE)  && (tray[subTrayNum][trayTail].evalEpoch==evalEpoch))
		{
			...
			{
				trayTail++;
				__MAX_DEBUG1__ dbg("DBG_USR3","trayTail=%d\n",trayTail);
			}
		}
	...
	command error_t PutTuples.putTuples(__TUPLE_PTR_TYPE__ inQueue, int8_t inHead, int8_t inTail, uint8_t inQueueSize)
	{
		...
		if (inHead > -1)
		{
			__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__  putTuples entered, evalEpoch of first tuple=%d\n",inQueue[inHead].evalEpoch);

			do
			{
				prevSubTrayNum = subTrayNum;
				subTrayNum = inQueue[inHead].evalEpoch % NUM_SUBTRAYS;
				__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__  putTuples: subTray=%d, prevSubTray=%d\n",subTrayNum,prevSubTrayNum);

				if (prevSubTrayNum!=subTrayNum)
				{
					__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__  putTuples: writing to a different subTray Number to the previous one\n");
					trayTail=0;

					while ((trayTail<SUBTRAY_SIZE) &&(tray[subTrayNum][trayTail].evalEpoch==inQueue[inHead].evalEpoch) )
					{
						atomic trayTail++;
						__MAX_DEBUG1__ dbg("DBG_USR3","__MODULE_NAME__  searching for next available slot, currently looking at: %d\n",trayTail);
					}
